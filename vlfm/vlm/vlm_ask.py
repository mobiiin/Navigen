...
try:
    from transformers import LlavaNextProcessor, LlavaNextForConditionalGeneration, AutoProcessor, PaliGemmaForConditionalGeneration
...
class VLMModel:
    ...
    def process_input(self, image1: np.ndarray, prompt: str, replace_word: str = "chair", image2: Optional[np.ndarray] = None) -> tuple:
        ...
        try:
            ...
            # Dynamically generate regex for found object
            found_pattern = rf"\*\*Have You Found the {replace_word}\?\*\*: (Yes|No)"
            found_match = re.search(found_pattern, response)
            found_ooi = found_match.group(1) if found_match else "Unknown"

            # Clear CUDA cache
            torch.cuda.empty_cache()
            print("foundddddddddddddd object of interest",found_ooi)
            
            return response, action_scores, found_ooi
        except Exception as e:
            print(f"Error in process_input: {e}")
            return f"Error: {str(e)}", {}, "Unknown"  # Ensure exactly three values are returned
...
class VLMModelClient:
    ...
    def process_input(self, image1: np.ndarray, prompt: str, replace_word: str = "chair", image2: Optional[np.ndarray] = None) -> tuple:
        ...
        try:
            ...
            return response["response"], response["action_scores"], response["found_ooi"]
        except Exception as e:
    ...
    class VLMModelServer(ServerMixin, VLMModel):
        def process_payload(self, payload: dict) -> dict:
            try:
                ...
                # Process the input
                response, action_scores, found_ooi = self.process_input(image1=image1, image2=image2, prompt=prompt, replace_word=replace_word)
                return {"response": response, "action_scores": action_scores , "found_ooi": found_ooi}
            except Exception as e:
                print(f"Error in process_payload: {e}")
                return {"response": f"Error: {str(e)}", "action_scores": {}, "found_ooi": "Unknown" }
